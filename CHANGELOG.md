...
### Added
- **Disabled Extension Banner**: A sticky warning banner now appears below the header whenever Moq is disabled, making the inactive state explicit
  - Banner shows "Moq is disabled — no rules are being applied." with an inline **Enable** button
  - Clicking the button re-enables the extension without navigating away
  - Banner uses the same color palette as existing header controls (gray background, green enable pill)
